...
# Optunaでチューニング
# VGGでKNN（全画像利用）
# tsneからBertを抜いて非圧縮で独立させる：除外
# KNNで自分自身を抜く
# tsneにtf-idf,bert,albert全て入れる、非圧縮Albertは残す
# 非圧縮Bertを20アイテムまで拡張（<-5アイテム）
# New: 次、vggを広げてみる20アイテム（<-5アイテム）
# MAP@12 (all): 0.032903
# MAP@12 (cold start): 0.007198
# rows exceeds upper limit of 10000 for a query
...
def _generate_candidates_desc_knn(transactions: pd.DataFrame, customers: np.ndarray, target_week: int, encoder: str):
    ...
    desc_knn = pd.read_csv(f'../input/ranking_features/item_features_{encoder}_class_tfidf_bert_albert.csv', dtype={'article_id': 'int32'})
...
def _generate_candidates_bert_knn(transactions: pd.DataFrame, customers: np.ndarray, target_week: int):
    ...
    n_neighbors = 20
    bert_knn = pd.read_csv(f'../input/ranking_features/article_bert_pairs_{n_neighbors}items.csv', dtype={'article_id': 'int32'}, 
                           # usecols=usecols
                          )
...
def generate_candidates_im_knn(transactions: pd.DataFrame, customers: np.ndarray, target_week: int):
    ...
    n_neighbors = 20
    im_knn = pd.read_parquet(f'../input/ranking_features/candidates_knn_vgg_{n_neighbors}items.parquet')
    im_knn['article_id'] = im_knn['article_id'].astype('int32')
    im_knn['knn_article_id'] = im_knn['knn_article_id'].astype('int32')
