...
def generate_candidates_als(transactions: pd.DataFrame, customers: np.ndarray, target_week: int):
    df = pd.read_parquet(f'../input/ranking_features/als_candidates_week{target_week}.parquet')
    df['customer_id'] = df['customer_id'].map(id_to_index_dict).astype('int32')
...
def make_data_df(transactions: pd.DataFrame, week: int, is_labeled: bool, use_customers: np.ndarray = None, metric_verbose: bool = True, compress_verbose: bool = False):
    ...
    data_dfs.append(generate_candidates_als(transactions, data_customers, week))
