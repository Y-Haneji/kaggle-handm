...
# Bert4Rec
# Bert KNN と 画像KNN と Bert4Rec を除外
# Gru4Rec
# New: 並列化
...
def generate_candidates_trending(transactions: pd.DataFrame, customers: np.ndarray, target_week: int, id_to_index_dict: dict):
...
def generate_candidates_recently(transactions: pd.DataFrame, customers: np.ndarray, target_week: int, id_to_index_dict: dict):
    '''直近の購入履歴から候補生成'''
    # week列のシフト(week=target_week => week=0)、リーク防止
    df = transactions.query("customer_id in @customers").copy()
    df['week'] = df['week'] - target_week
...
def generate_candidates_popular(transactions: pd.DataFrame, customers: np.ndarray, target_week: int, id_to_index_dict: dict):
    '''販売数の多い商品から候補生成'''
...
def generate_candidates_bin_popular(transactions: pd.DataFrame, customers: np.ndarray, target_week: int, id_to_index_dict: dict):
    ...
    bin_popular_items = bin_popular_items[bin_popular_items['rank'] <= 30]
...
def generate_candidates_lastw(transactions: pd.DataFrame, customers: np.ndarray, target_week: int, id_to_index_dict: dict):
...
def generate_candidates_pair(transactions: pd.DataFrame, customers: np.ndarray, target_week: int, id_to_index_dict: dict):
...
def generate_candidates_collabo(transactions: pd.DataFrame, customers: np.ndarray, target_week: int, id_to_index_dict: dict):
    '''コラボ商品の購入履歴から候補生成'''
...
def _generate_candidates_desc_knn(transactions: pd.DataFrame, customers: np.ndarray, target_week: int, id_to_index_dict: dict, encoder: str, ):
...
def _generate_candidates_albert_knn(transactions: pd.DataFrame, customers: np.ndarray, target_week: int, id_to_index_dict: dict):
    # week列のシフト(week=target_week => week=0)、リーク防止
    df = transactions.query("customer_id in @customers").copy()
    df['week'] = df['week'] - target_week
    df = df.query("week >= 1")
...
def _generate_candidates_bert_knn(transactions: pd.DataFrame, customers: np.ndarray, target_week: int, id_to_index_dict: dict):
    # week列のシフト(week=target_week => week=0)、リーク防止
    df = transactions.query("customer_id in @customers").copy()
    df['week'] = df['week'] - target_week
    df = df.query("week >= 1")
...
def generate_candidates_desc_knn(transactions: pd.DataFrame, customers: np.ndarray, target_week: int, id_to_index_dict: dict):
    '''商品説明文から候補生成'''
    desc_knns = []
    
    desc_knns.append(_generate_candidates_desc_knn(transactions, customers, target_week, id_to_index_dict, encoder='tsne'))
    # desc_knns.append(_generate_candidates_desc_knn(transactions, customers, target_week, id_to_index_dict, encoder='umap'))
    desc_knns.append(_generate_candidates_albert_knn(transactions, customers, target_week, id_to_index_dict))
    # desc_knns.append(_generate_candidates_bert_knn(transactions, customers, target_week, id_to_index_dict))
...
def generate_candidates_im_knn(transactions: pd.DataFrame, customers: np.ndarray, target_week: int, id_to_index_dict: dict):
    '''画像の埋め込みベクトルから候補生成'''
    # week列のシフト(week=target_week => week=0)、リーク防止
    df = transactions.query("customer_id in @customers").copy()
    df['week'] = df['week'] - target_week
...
def generate_candidates_sequence_word2vec(transactions: pd.DataFrame, customers: np.ndarray, target_week: int, id_to_index_dict: dict):
...
def _generate_candidates_lstm(transactions: pd.DataFrame, customers: np.ndarray, target_week: int, id_to_index_dict: dict, model: str):
...
def generate_candidates_lstm(transactions: pd.DataFrame, customers: np.ndarray, target_week: int, id_to_index_dict: dict):
    return _generate_candidates_lstm(transactions, customers, target_week, id_to_index_dict, model='gru4rec')
...
def make_data_df(transactions: pd.DataFrame, week: int, id_to_index_dict: dict, is_labeled: bool, use_customers: np.ndarray = None, metric_verbose: bool = True, compress_verbose: bool = False):
    '''ランク学習モデルへの入力データ作成'''
    ...
    # 候補作り
    data_dfs = []
    data_dfs.append(generate_candidates_trending(transactions, data_customers, week, id_to_index_dict))
    data_dfs.append(generate_candidates_recently(transactions, data_customers, week, id_to_index_dict))
    data_dfs.append(generate_candidates_popular(transactions, data_customers, week, id_to_index_dict))
    data_dfs.append(generate_candidates_bin_popular(transactions, data_customers, week, id_to_index_dict))
    data_dfs.append(generate_candidates_lastw(transactions, data_customers, week, id_to_index_dict))
    data_dfs.append(generate_candidates_pair(transactions, data_customers, week, id_to_index_dict))
    data_dfs.append(generate_candidates_collabo(transactions, data_customers, week, id_to_index_dict))
    data_dfs.append(generate_candidates_desc_knn(transactions, data_customers, week, id_to_index_dict))
    # data_dfs.append(generate_candidates_im_knn(transactions, data_customers, week, id_to_index_dict))
    data_dfs.append(generate_candidates_sequence_word2vec(transactions, data_customers, week, id_to_index_dict))
    data_dfs.append(generate_candidates_lstm(transactions, data_customers, week, id_to_index_dict))
...
def extract_top_sr(test_df: pd.DataFrame, pred: np.ndarray):
    ...
    pred_df['predict_score'] = pred
    pred_df = pred_df.sort_values('predict_score', ascending=False).drop_duplicates(['customer_id', 'article_id'], keep='first').reset_index(drop=True)
    pred_df['rank'] = pred_df.groupby('customer_id')['predict_score'].rank('min', ascending=False)
    pred_df = pred_df[pred_df['rank'] <= 12]
    
    # pred_df['article_id'] = le.inverse_transform(pred_df['article_id'])
    pred_df['article_id'] = ' 0' + pred_df['article_id'].astype(str)
    
    top_sr = pred_df.groupby('customer_id')['article_id'].sum()
...
def main():
    ...
    # tr_df = make_data_df(transactions, 2, id_to_index_dict, is_labeled=True, metric_verbose=False, compress_verbose=False)
    ...
    # val_df = make_data_df(transactions, 1, id_to_index_dict, is_labeled=True, metric_verbose=False, compress_verbose=False)
    # val_df = val_df.sort_values('customer_id').reset_index(drop=True)    
    # val_query = val_df.groupby('customer_id')['customer_id'].count().to_list()
    # dval = lgb.Dataset(val_df[cols], reference=dtrain, label=val_df['label'], group=val_query)
...
#         tr_df = make_data_df(transactions, w, id_to_index_dict, is_labeled=True, metric_verbose=True, compress_verbose=compress_verbose)
#         val_df = make_data_df(transactions, w-1, id_to_index_dict, is_labeled=True, metric_verbose=False, compress_verbose=False)
...
#     # predict val data
#     val_df = make_data_df(transactions, 0, id_to_index_dict, is_labeled=False, metric_verbose=True, compress_verbose=False)
...
#     tr_df = make_data_df(transactions, 0, id_to_index_dict, is_labeled=True, metric_verbose=True, compress_verbose=False)
...
#     gc.collect()
    ...
    # バッチ処理
    def predict_test_customers(i):
        if i == int(np.ceil(len(test_customers) / BATCH_SIZE))-1:
        ...
        if i == 0:  # meric_verbose=True
            test_df = make_data_df(transactions, -1, id_to_index_dict, is_labeled=False, use_customers=test_customers_batch, metric_verbose=True, compress_verbose=False)
        else:       # metric_verbose=False
            test_df = make_data_df(transactions, -1, id_to_index_dict, is_labeled=False, use_customers=test_customers_batch, metric_verbose=False, compress_verbose=False)

        all_weeks = oof_weeks + [0]
        pred = predict(test_df, all_weeks)
        pred_sr = extract_top_sr(test_df, pred)
        return pred_sr

    # single process execution
    preds = []
    for i in tqdm(range(int(np.ceil(len(test_customers) / BATCH_SIZE)))):
        preds.append(predict_test_customers(i))

    # # multi process execution
    # cpus = cpu_count(logical=False)
    # # cpus = 4
    # print('cpu(core): ', cpus)
    # with ThreadPoolExecutor(max_workers=cpus//4, thread_name_prefix=__file__) as executor:
    #     preds = executor.map(predict_test_customers, range(int(np.ceil(len(test_customers) / BATCH_SIZE))))
    # preds = list(preds)
    # # preds = Parallel(n_jobs=cpus, verbose=0)( [delayed(predict_test_customers)(i) for i in range(int(np.ceil(len(test_customers) / BATCH_SIZE)))] )
