...
# Bert4Rec：除外
# Gru4Rec：除外
# Optunaでチューニング❌NDCGを最小化してしまっている
# New: 非圧縮Word2Vec
# MAP@12 (all): 0.032974
# MAP@12 (cold start): 0.007380
EXP = '043'
...
def generate_candidates_sequence_word2vec(transactions: pd.DataFrame, customers: np.ndarray, target_week: int):
    ...
    word2vec_knn = pd.read_parquet(f'../input/ranking_features/word2vec_without_compress_5items.parquet')
    df = pd.merge(df, word2vec_knn, on='article_id').rename(columns={'knn_distance': 'sequence_word2vec_distance'})
    
    # n_neighbors = 5    
    # dfs = []
    # for i in range(n_neighbors):
    #     dfs.append(df.merge(word2vec_knn[['article_id', f'knn_article_id_{i}', f'knn_distance_{i}']])
    #                     .rename(columns={f'knn_article_id_{i}': 'knn_article_id', f'knn_distance_{i}': 'sequence_word2vec_distance'}))
    # df = pd.concat(dfs, axis=0, ignore_index=True)
    df = df.drop('article_id', axis=1).rename(columns={'knn_article_id': 'article_id'})
    df['sequence_word2vec_count'] = df['count'] * abs(1-df['sequence_word2vec_distance'])
